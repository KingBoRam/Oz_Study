...
function userInfo(name, age, favoriteColor) {
  let userName = name;
  let userAge = age;
  let userFavoriteColor = favoriteColor;
  let text = '';
  if (userAge < 18) {
    text = `${userName}님은 18세 미만입니다.`;
    return text;
  } else if (userAge >= 18) {
    text = `안녕하세요, ${userName}님! 당신은 ${userAge}살이고, 좋아하는 색상은 ${userFavoriteColor}입니다.`;
    return text;
  }
}
