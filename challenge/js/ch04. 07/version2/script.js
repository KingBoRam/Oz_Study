...
generateWinningNumbers.addEventListener('click', () => {
  ...
  */
  ...
  for (let i = 0; i < winningNumbers.length; i++) {
    winningNumContainer.append(displayNumbers(winningNumbers[i]));
  }
});
...
generateLottoNumbers.addEventListener('click', () => {
  ...
  */
  if (winningNumbers.length !== 6) {
    ...
    if (lottoNumContainer.innerHTML !== '') {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      numberContainer.classList.add('number-set');
      for (let j = 0; j < numbers.length; j++) {
        numberContainer.append(displayNumbers(numbers[j], winningNumbers));
      }
      lottoNumbers.append(numberContainer);
    }
  }
});
